...
class GradCAM(object):
    ...
    def _get_gradients_and_filters(self,
        ...
        if use_guided_grads:
            grads = tf.cast(conv_outputs > 0, "float32") * tf.cast(
    ...
    def heatmap(self, inputs, class_idx, use_guided_grads=False):
        output, grad = self._get_gradients_and_filters(
...
class Predictor(object):
    ...
    def _load_model(self):
        ...
        # load parameters
        with open(self.model_prefix.replace('_best_train_model','') + "_model_params.json") as json_file:
            metadata = json.load(json_file)

        self.gonames = np.asarray(metadata['gonames'])
        self.goterms = np.asarray(metadata['goterms'])
        self.thresh = 0.01 * np.ones(len(self.goterms))
    ...
    def predict_from_PDB_dir(self, dir_name, cmap_thresh=10.0):
        ...
        self.chain2path = {
            pdb_fn.split('/')[-1].split('.')[0]: pdb_fn
            for pdb_fn in pdb_fn_list
        }
        self.test_prot_list = list(self.chain2path.keys())
        self.Y_hat = np.zeros((len(self.test_prot_list), len(self.goterms)),
                              dtype=float)
        self.goidx2chains = {}
        self.prot2goterms = {}
        self.data = {}
        for i, chain in enumerate(self.test_prot_list):
            A, S, seqres = self._load_cmap(self.chain2path[chain],
    ...
    def predict_from_fasta(self, fasta_fn):
        ...
        for i, chain in enumerate(tqdm(self.test_prot_list)):
    ...
    def export_csv(self, output_fn, verbose):
        with open(output_fn, 'w') as csvFile:
            ...
            if verbose:
                print('Protein', 'GO-term/EC-number', 'Score',
                      'GO-term/EC-number name')
            for prot in self.prot2goterms:
                sorted_rows = sorted(self.prot2goterms[prot],
                                     key=lambda x: x[2],
                                     reverse=True)
    ...
    def compute_GradCAM(self,
        ...
        for go_indx in self.goidx2chains:
            ...
            print("### Computing gradCAM for ", self.gonames[go_indx],
                  '... [# proteins=', len(pred_chains), ']')
            for chain in pred_chains:
                ...
                self.pdb2cam[chain]['saliency_maps'].append(
                    gradcam.heatmap(
                        self.data[chain][0],
                        go_indx,
                        use_guided_grads=use_guided_grads).tolist())
